...
def test_missing_secret_raises():
    with pytest.raises(ConfigError, match="TRADING212_SECRET_KEY"):
        load_config(env={"TRADING212_API_KEY": "k"})


def test_live_default_base_url():
    cfg = load_config(env={"TRADING212_API_KEY": "k", "TRADING212_SECRET_KEY": "s"})
...
def test_demo_env_switches_host():
    cfg = load_config(env={"TRADING212_API_KEY": "k", "TRADING212_SECRET_KEY": "s", "TRADING212_ENV": "demo"})
...
def test_base_url_override_wins():
    cfg = load_config(
        env={
            "TRADING212_API_KEY": "k", "TRADING212_SECRET_KEY": "s",
            "TRADING212_ENV": "demo",
...
def test_invalid_env_rejected():
    with pytest.raises(ConfigError, match="TRADING212_ENV"):
        load_config(env={"TRADING212_API_KEY": "k", "TRADING212_SECRET_KEY": "s", "TRADING212_ENV": "prod"})


@pytest.mark.parametrize("value", ["1", "true", "TRUE", "yes", "on"])
def test_allow_writes_truthy(value):
    cfg = load_config(env={"TRADING212_API_KEY": "k", "TRADING212_SECRET_KEY": "s", "TRADING212_ALLOW_WRITES": value})
...
def test_timeout_parses_float():
    cfg = load_config(env={"TRADING212_API_KEY": "k", "TRADING212_SECRET_KEY": "s", "TRADING212_TIMEOUT_SECONDS": "12.5"})
    assert cfg.timeout_seconds == 12.5


def test_timeout_invalid_rejected():
    with pytest.raises(ConfigError, match="TIMEOUT"):
        load_config(env={"TRADING212_API_KEY": "k", "TRADING212_SECRET_KEY": "s", "TRADING212_TIMEOUT_SECONDS": "soon"})


def test_redacted_hides_key():
    cfg = load_config(env={"TRADING212_API_KEY": "super-key", "TRADING212_SECRET_KEY": "super-secret"})
    redacted = cfg.redacted()
    assert "super-key" not in str(redacted)
